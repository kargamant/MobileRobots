...
namespace Robots
{
	...
	public:
		static double PI;
		Sensor(int radius = 1, std::pair<int, int> direction = { 1, 1 }, ViewAngles angle = ViewAngles::tau, double energy = 2, bool state = true, Priority priority = Priority::high, int cost = 2000) :radius(radius), direction(direction), angle(angle), Module(energy, state, priority, cost) { isSensor = true; } //automatically sets priority to medium
